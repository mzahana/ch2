...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if Crawling: # CRAWL MODE
            ...
            print "Current: ", x, y, z
            print "Shifting: ", -10, 0, 0
            flag = sm.xyzShiftWayPoints(-100.0,0.0,0.0,10,velocity)
            sm.client.wait_for_result()
            Arrived = False
            while not Arrived:
                Q =  sm.jointPosition
                ...
                if sm.anyButton():
                    ...
                    print "Current: ", xNow, yNow, zNow
                elif sm.client.get_state() == 3: # SUCCEEDED
                    Arrived = True # Break from Arrived loop but retry CRAWL mode
                else:
                    xNow,yNow,zNow = sm.wristFK(sm.jointPosition[0],sm.jointPosition[1],sm.jointPosition[2])
